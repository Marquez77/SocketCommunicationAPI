...
public class LongDataParsingTest {
    public static void main(String[] args) throws IOException {
        ...
        server.registerHandler((client, send, response) -> {
            System.out.println("server1: " + send.nextLong());
        });
        server.start();

        UDPEchoServer server2 = new UDPEchoServer(8081, LoggerFactory.getLogger("server2"));
        server2.registerHandler((client, send, response) -> {
            System.out.println("server2: " + send.nextLong());
        });
        server2.start();

        server.sendDataAndReceive(new InetSocketAddress("localhost", 8081), new UDPEchoSend(System.currentTimeMillis()));
    }
}
